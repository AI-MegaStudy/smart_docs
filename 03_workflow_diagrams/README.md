...
`index.html`의 기본 화면은 PPT 삽입용 PNG/SVG 산출물을 한글 제목과 함께 보여줍니다. 기존의 작은 화면별 플로우도 상단 탭에서 `고객 예약웹`, `점주앱`, `상태·트랜잭션`으로 계속 확인할 수 있습니다.
...
- `PPT용 이미지`: 완성된 대형 PNG/SVG 다이어그램을 한글 제목, 설명, 미리보기와 함께 표시합니다.
- `고객 예약웹`: 고객 웹 화면 단위의 작은 유저플로우를 표시합니다.
- `점주앱`: 점주앱 화면 단위의 작은 유저플로우를 표시합니다.
- `상태·트랜잭션`: 상태 변경과 DB 갱신 지점을 화면별 플로우 형태로 표시합니다.
